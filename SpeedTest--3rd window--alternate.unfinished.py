...
from tkinter import *
#from PIL import Image, ImageTk     # Issue installing PIL on Windows, app works fine without this. Mac?
...
class Main(tk.Frame):
    ...
    def setDefaults(self):
        ...
        #Background image
        background_image=tk.PhotoImage(file='bg_image.gif')
    ...
    def createWidgets(self):
        ...
        self.startButton.configure(highlightthickness=2)
        self.startButton.configure(command = self.changeToTest)
        self.startButton.pack(side="top", padx=110, pady=132)

        self.resultsButton = tk.Button(self.root, text = 'View Results')
        self.resultsButton.configure(highlightthickness=2)
        self.resultsButton.configure(command = self.showResults)
    ...
    def changeToTest(self):
        ...
        self.rThread.start() # Show results page

    def changeToResults(self, lock):
        with lock:
            print("Results thread started")
            self.resultsButton.pack(pady=20)
    def showResults(self):
        
        # Clear off widgets
        self.resultsButton.forget()     # clear off results button
        self.infoLabel.forget()
        self.ProgBar.forget()
        self.ProgBar.forget()
        
        self.resultsLabel = tk.Label(text = 'Download rate')
    ...
    def startDownload(self, lock):
        with lock:
            ...
            self.infoLabel.pack(pady=20)                                # Display download label
    ...
    def startDTest(self, fileToDL, fileToUL, FileSize):
        ...
        self.ProgBar = ttk.Progressbar(self.root, orient = 'horizontal', length = 250,\
                                       mode = 'determinate', maximum = FileSize)
    ...
    def startUpload(self, lock):
        with lock:
            ...
            self.infoLabel.pack(pady=20)                                # Display upload label
